...
`go-ruby-toml/toml` reimplements Ruby's `toml` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-toml/toml` directly, with no Ruby runtime;
